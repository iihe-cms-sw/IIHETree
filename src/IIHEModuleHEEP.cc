...
void IIHEModuleHEEP::beginJob(){
  ...
  addBranch("HEEP_ESTrack_eta") ;
  addBranch("HEEP_ESTrack_phi") ;
  addBranch("HEEP_ESTrack_x"  ) ;
  addBranch("HEEP_ESTrack_y"  ) ;
  addBranch("HEEP_ESTrack_z"  ) ;
  setBranchType(kVectorVectorInt) ;
  addBranch("HEEP_ESTrack_side" ) ;
  addBranch("HEEP_ESTrack_plane") ;
  addBranch("HEEP_ESTrack_valid") ;
  ...
  HEEPCutflow_41_ID_->addCut( (HEEPCutBase*) new HEEPCut_SigmaIetaIeta(kHC41, prefix_41 + "_SigmaIetaIeta", this)) ;
  ...
  HEEPCutflow_50_50ns_ID_->addCut( (HEEPCutBase*) new HEEPCut_SigmaIetaIeta (kHC50_50ns, prefix_50_50ns + "_SigmaIetaIeta", this)) ;
  ...
  HEEPCutflow_50_25ns_ID_->addCut( (HEEPCutBase*) new HEEPCut_SigmaIetaIeta (kHC50_25ns, prefix_50_25ns + "_SigmaIetaIeta", this)) ;
  ...
  HEEPCutflow_50_ID_->addCut( (HEEPCutBase*) new HEEPCut_SigmaIetaIeta(kHC50, prefix_50 + "_SigmaIetaIeta", this)) ;
  ...
  HEEPCutflow_51_ID_->addCut( (HEEPCutBase*) new HEEPCut_SigmaIetaIeta(kHC51, prefix_51 + "_SigmaIetaIeta", this)) ;
...
CHOOSE_RELEASE_END CMSSW_7_0_6_patch1 CMSSW_6_2_5 CMSSW_6_2_0_SLHC23_patch1 CMSSW_5_3_11       */
  ...
  for(reco::GsfElectronCollection::const_iterator gsfiter = electrons.begin() ; gsfiter!=electrons.end() ; ++gsfiter){
    ...
    // Try to match the preshower hits to the track
    // Inspired by https://github.com/alphatsai/ESAlignment/blob/master/AlignmentTool/ESAlignTool/src/ESAlignTool.cc#L1146
    ESHandle<MagneticField> theMagField ;
    iSetup.get<IdealMagneticFieldRecord>().get(theMagField) ;
    ESHandle<GlobalTrackingGeometry> theTrackingGeometry ;
    iSetup.get<GlobalTrackingGeometryRecord>().get( theTrackingGeometry ) ;
    
    align::PositionType *ES_Oap ;
    align::RotationType woRotate ;
    
    std::vector<float> ES_x ;
    std::vector<float> ES_y ;
    std::vector<float> ES_z ;
    std::vector<float> ES_eta ;
    std::vector<float> ES_phi ;
    std::vector<int  > ES_side  ;
    std::vector<int  > ES_plane ;
    std::vector<int  > ES_valid ;
    
    for(unsigned int i1=0 ; i1<2 ; ++i1){
      int iz = (i1==0) ? -1 : 1 ; // -1 for -ve, +1 for +ve
      for(unsigned int i2=0 ; i2<2 ; ++i2){
        int ip = i2+1 ; // 1 for front, 2 for rear
        ES_Oap = new align::PositionType(ES_OX_[i1][i2],ES_OY_[i1][i2],ES_OZ_[i1][i2]) ;
        SteppingHelixPropagator ESpropa(theMagField.product()) ;
        Plane::PlanePointer Plane_ES = Plane::build(*ES_Oap,woRotate) ;
        
        reco::GsfTrackRef track = gsfiter->gsfTrack() ;
        if(track->charge()!=0){
          reco::TransientTrack TTrack(*track, theMagField.product(), theTrackingGeometry) ;
          ...
          if(ipTSOS.isValid() && TTrack.impactPointStateAvailable()){
            ...
            if(ES_prediction.isValid()){
              ES_eta.push_back(ES_prediction.globalPosition().eta()) ;
              ES_phi.push_back(ES_prediction.globalPosition().phi()) ;
              ES_x  .push_back(ES_prediction.globalPosition().  x()) ;
              ES_y  .push_back(ES_prediction.globalPosition().  y()) ;
              ES_z  .push_back(ES_prediction.globalPosition().  z()) ;
              ES_valid.push_back(1) ;
            }
            else{
              ES_eta.push_back(0) ;
              ES_phi.push_back(0) ;
              ...
              ES_y  .push_back(0) ;
              ES_z  .push_back(0) ;
              ES_valid.push_back(0) ;
            }
            ES_side .push_back(iz) ;
            ES_plane.push_back(ip) ;
          }
        }
      }
    }
    store("HEEP_ESTrack_eta"  , ES_eta  ) ;
    store("HEEP_ESTrack_phi"  , ES_phi  ) ;
    store("HEEP_ESTrack_x"    , ES_x    ) ;
    store("HEEP_ESTrack_y"    , ES_y    ) ;
    store("HEEP_ESTrack_z"    , ES_z    ) ;
    store("HEEP_ESTrack_side" , ES_side ) ;
    store("HEEP_ESTrack_plane", ES_plane) ;
    store("HEEP_ESTrack_valid", ES_valid) ;
    
    // Try to add info about rechit in the SC 
    // Strongly inspired by: http://cmssw.cvs.cern.ch/cgi-bin/cmssw.cgi/UserCode/DaveC/src/printPhoton.cc
    //Crystal variables
      ...
      for(int ix=0 ; ix<64 ; ++ix){
        for(int iy=0 ; iy<64 ; ++iy){
          for(int is=0 ; is<64 ; ++is){
            ESDetId esid = ESDetId(is,ix,iy,ip,iz) ;
            if(!esid.validDetId(is,ix,iy,ip,iz)) continue ;
            float ESX = geometry->getPosition(esid).x() ;
            float ESY = geometry->getPosition(esid).y() ;
            float ESZ = geometry->getPosition(esid).z() ;
            if(ESX<xMin) xMin = ESX ;
            if(ESX>xMax) xMax = ESX ;
            if(ESY<yMin) yMin = ESY ;
            if(ESY>yMax) yMax = ESY ;
            if(ESZ<zMin) zMin = ESZ ;
            if(ESZ>zMax) zMax = ESZ ;
          }
        }
      }
      ES_OX_[i1][i2] = 0.5*(xMin+xMax) ;
      ES_OY_[i1][i2] = 0.5*(yMin+yMax) ;
      ES_OZ_[i1][i2] = 0.5*(zMin+zMax) ;
    }
  }
}
